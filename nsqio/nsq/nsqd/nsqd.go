...
func New(opts *Options) (*NSQD, error) {
	...
	//通过连接和请求超时时间创建HTTP客户端
	httpcli := http_api.NewClient(nil, opts.HTTPClientConnectTimeout, opts.HTTPClientRequestTimeout)
	n.ci = clusterinfo.New(n.logf, httpcli)		//创建客户端集群信息

	n.lookupPeers.Store([]*lookupPeer{})

	//这里使用了 atomic.Value 来存储 opts、lookupPeers 以及 errValue
	//	轻量级的实现了数据的原子性
	n.swapOpts(opts)
	n.errValue.Store(errStore{})

	err = n.dl.Lock()		//文件锁上锁，判断该路径是否正在被其他进程使用
	if err != nil {
		return nil, fmt.Errorf("--data-path=%s in use (possibly by another instance of nsqd)", dataPath)
	}

	if opts.MaxDeflateLevel < 1 || opts.MaxDeflateLevel > 9 {	//判断文件压缩级别是否合理
		return nil, errors.New("--max-deflate-level must be [1,9]")
	}

	if opts.ID < 0 || opts.ID >= 1024 {			//判断默认进程id是否在预定范围
		return nil, errors.New("--node-id must be [0,1024)")
	}

	if opts.StatsdPrefix != "" {	//设置状态前缀为服务域名
		var port string
	...
	//配置TLS
	if opts.TLSClientAuthPolicy != "" && opts.TLSRequired == TLSNotRequired {	//判断tls客户端身份验证
		opts.TLSRequired = TLSRequired
	}

	tlsConfig, err := buildTLSConfig(opts)	//获取构建的tls配置
	if err != nil {
		return nil, fmt.Errorf("failed to build TLS config - %s", err)
	}
	if tlsConfig == nil && opts.TLSRequired != TLSNotRequired {		//判断tls是否配置成功
...
func (n *NSQD) LoadMetadata() error {
	...
	fn := newMetadataFile(n.getOpts())	//获取数据存储路径
	...
	var m meta
	err = json.Unmarshal(data, &m)	//解析为元数据
	if err != nil {
		return fmt.Errorf("failed to parse metadata in %s - %s", fn, err)
	}

	for _, t := range m.Topics {		//遍历获取
		if !protocol.IsValidTopicName(t.Name) {	//检查Topic是否合法
			n.logf(LOG_WARN, "skipping creation of invalid topic %s", t.Name)
			continue
		}
		topic := n.GetTopic(t.Name)	//使用GetTopic函数通过名字获得topic对象
		if t.Paused {			//判断当前topic对象是否处于暂停状态，是的话调用Pause函数暂停topic
			topic.Pause()
		}
		for _, c := range t.Channels {	//获取当前topic下所有的channel，并且遍历channel，执行的操作与topic基本一致
...
func (n *NSQD) PersistMetadata() error {
	//获取存储路径 - dataPath + nsqd.dat，然后遍历获取每个 topic 的 name 和 pause 状态，最后同步写入，完成持久化
	...
	err = writeSyncFile(tmpFileName, data)	//先创建扩展名为tmp的临时文件
	if err != nil {
		return err
	}
	err = os.Rename(tmpFileName, fileName)	//写入内容后并保存后再调用atomicRename函数将tmp文件重命名为nsqd.%d.dat。
	if err != nil {
		return err
	}
...
func (n *NSQD) queueScanLoop() {
	...
	workTicker := time.NewTicker(n.getOpts().QueueScanInterval)		//设置扫描队列间隔定时器
	refreshTicker := time.NewTicker(n.getOpts().QueueScanRefreshInterval)	//设置队列扫描刷新间隔定时器

	channels := n.channels()	//所有Topic通道
	n.resizePool(len(channels), workCh, responseCh, closeCh)	//调整queueScanWorker池大小


	/*当 refreshTicker.C 赢得 select 的竞争的时候，queueScanLoop 会根据当前的情况，
	动态的增删 queueScanWorker，queueScanWorker 用于处理每个 channel 中延迟发送的 message
	和 timeout 的 message；当 workTicker.C 赢得 select 的竞争的时候，queueScanLoop 会计算
	当前的 dirtyPercent，也就是存在需要延迟发送/timeout 的channel 比率，若该比率大于 opts
	中的额定值，queueScanLoop 会暂停 loop 等待 dirtyPercent 降低*/
	...
	for {
		...
		case <-refreshTicker.C:		//扫描刷新，调整queueScanWorker goroutines池的大小
			channels = n.channels()
			n.resizePool(len(channels), workCh, responseCh, closeCh)
			continue
		case <-n.exitChan:		//检测是否退出
			goto exit
		}
	...
	loop:
		...
		//若该比率大于 opts	中的额定值，queueScanLoop 会暂停， 执行loop， 等待 dirtyPercent 降低
